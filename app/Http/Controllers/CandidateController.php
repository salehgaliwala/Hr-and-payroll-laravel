...
class CandidateController extends Controller
{
    ...
    /**
     * Mark candidate as unsuccessful (rejected) and send notifications.
     */
    public function markAsUnsuccessful(Candidate $candidate)
    {
        if (!in_array($candidate->created_by, getCompanyAndUsersId())) {
            return redirect()->back()->with('error', 'You do not have permission to update this candidate');
        }
        ...
        // Check if already marked as unsuccessful and notification already sent
        if ($candidate->rejection_notification_sent) {
            return redirect()->back()->with('warning', __('This candidate has already received a rejection notification. Use the resend option if needed.'));
        }

        // Update candidate status and rejection info
        $candidate->update([
            'status' => 'Rejected',
            'unsuccessful_by' => Auth::id(),
            'unsuccessful_at' => now(),
            'rejection_notification_sent' => false,
        ]);

        // Dispatch event to send rejection notifications
        CandidateRejected::dispatch($candidate, Auth::id());

        return redirect()->back()->with('success', __('Candidate marked as unsuccessful and rejection notifications sent.'));
    }

    /**
     * Resend rejection notification for a candidate.
     */
    public function resendRejectionNotification(Candidate $candidate)
    {
        if (!in_array($candidate->created_by, getCompanyAndUsersId())) {
            return redirect()->back()->with('error', 'You do not have permission to update this candidate');
        }
        ...
        if ($candidate->status !== 'Rejected') {
            return redirect()->back()->with('error', __('Candidate must be in Rejected status to resend rejection.'));
        }

        // Reset the flag so the listener sends the notification again
        $candidate->update(['rejection_notification_sent' => false]);

        CandidateRejected::dispatch($candidate, Auth::id());

        return redirect()->back()->with('success', __('Rejection notification resent successfully.'));
    }
