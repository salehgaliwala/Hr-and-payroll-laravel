...
export default function TimeEntries() {
  ...
  const handleFormSubmit = (formData: any) => {
    if (formMode === 'create') {
      if (!globalSettings?.is_demo) {
        toast.loading(t('Creating timesheet...'));
      }
      ...
      router.post(route('hr.time-entries.store'), formData, {
        ...
        onError: (errors) => {
          ...
          if (typeof errors === 'string') {
            ...
            toast.error(`Failed to create timesheet: ${Object.values(errors).join(', ')}`);
          }
        }
      });
    } else if (formMode === 'edit') {
      if (!globalSettings?.is_demo) {
        toast.loading(t('Updating timesheet...'));
      }
      ...
      router.put(route('hr.time-entries.update', currentItem.id), formData, {
        ...
        onError: (errors) => {
          ...
          if (typeof errors === 'string') {
            ...
            toast.error(`Failed to update timesheet: ${Object.values(errors).join(', ')}`);
          }
        }
      });
    }
  ...
  const handleDeleteConfirm = () => {
    if (!globalSettings?.is_demo) {
      toast.loading(t('Deleting timesheet...'));
    }
  ...
  const handleStatusUpdate = (timeEntry: any, status: string) => {
    ...
    if (!globalSettings?.is_demo) {
      toast.loading(`${statusText} timesheet...`);
    }
    ...
    router.put(route('hr.time-entries.update-status', timeEntry.id), { 
      ...
      onError: (errors) => {
        ...
        if (typeof errors === 'string') {
          ...
          toast.error(`Failed to update timesheet status: ${Object.values(errors).join(', ')}`);
        }
      }
    });
  };
