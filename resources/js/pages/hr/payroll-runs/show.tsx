        ...
        <CardHeader>
          ...
          <div className="mt-2 p-3 bg-blue-50 dark:bg-blue-900/20 rounded-md space-y-2">
            <p className="text-xs text-blue-800 dark:text-blue-200 font-medium">
              {t('Gross Pay Formula')} : <span className="font-mono">Total Earnings (Basic Salary + Component Earnings) - LOP Deduction - Unpaid Leave Deduction + Overtime Earnings</span>
            </p>
            <p className="text-xs text-blue-800 dark:text-blue-200 font-medium">
              {t('Net Salary Formula')} : <span className="font-mono">Gross Pay - Total Component Deductions</span>
            </p>
            <p className="text-xs text-blue-800 dark:text-blue-200 font-medium">
              {t('LOP Deduction Formula')} : <span className="font-mono">(Basic Salary / Total Working Days) × LOP Days</span>
            </p>
          </div>
        </CardHeader>
            ...
            <CrudTable
              columns={[
                ...
                // --- Earnings ---
                {
                  ...
                  render: (value: number) => (
                    <span className="font-mono text-gray-900 dark:text-gray-100">{window.appSettings?.formatCurrency(value)}</span>
                  )
                },
                {
                ...
                },
                {
                  ...
                  render: (value: number) => (
                    <span className="font-mono text-green-600 font-semibold">{value || 0}</span>
                  )
                },
                {
                  key: 'lop_days',
                  label: t('LOP Days'),
                  render: (value: number) => (
                    <span className="font-mono text-red-600 font-semibold">{value || 0}</span>
                  )
                },
                // --- Deductions ---
                {
                  key: 'lop_deduction',
                  label: t('LOP Deduction'),
                  render: (value: number) => (
                ...
                },
                {
                  ...
                  label: t('Unpaid Leave Days'),
                  render: (value: number) => (
                    <span className="font-mono text-orange-600">{value || 0}</span>
                  )
                },
                {
                  key: 'unpaid_leave_deduction',
                  label: t('Unpaid Leave Deduction'),
                  render: (value: number) => (
                    <span className="font-mono text-orange-600">{window.appSettings?.formatCurrency(value || 0)}</span>
                  )
                },
                {
                  key: 'overtime_amount',
                ...
                // --- Final ---
                {
                  ...
                  label: t('Gross Pay'),
                  render: (value: number) => (
                    <span className="font-mono font-bold text-green-700">{window.appSettings?.formatCurrency(value)}</span>
                  )
                },
                {
                ...
                },
                {
                  ...
                  label: t('Net Pay'),
                  render: (value: number) => (
                    <span className="font-mono font-bold text-blue-700">{window.appSettings?.formatCurrency(value)}</span>
                  )
                }
              ]}
