...
export default function EmployeeCreate() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    router.post(route('hr.employees.store'), submitData, {
      ...
      onError: (errors) => {
        setErrors(errors);
        if (!isDemo) toast.error(t('Please correct the errors in the form'));
      },
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-2">
                <Label htmlFor="banking_option" required>{t('Payment Option')}</Label>
                <Select
                ...
                >
                  <SelectTrigger className={errors.banking_option ? 'border-red-500' : ''}>
                    <SelectValue placeholder={t('Select Payment Option')} />
                  </SelectTrigger>
                  <SelectContent>
            ...
            {formData.banking_option === 'bank_account' ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-2">
                  <Label htmlFor="wallet_provider" required>{t('Select Wallet Provider')}</Label>
                  <Select
